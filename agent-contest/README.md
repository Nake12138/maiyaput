...
4. **缺少 UI 角色，交互与样式设计压力集中在产品**：当前团队没有专职 UI 设计师，页面的交互流程、信息层级、组件样式规范需要产品经理自行设计；既额外占用产品经理的产出时间，又因为缺乏专业设计判断，容易出现交互不够直观、视觉层级混乱、可访问性与一致性考虑不足等「不够合理」的设计。
5. **需求验证依赖研发**：静态原型无法自测，很多交互问题要等到前端开发阶段才暴露。

### 1.3 影响范围

- **能力边界**：原流程单人产出物局限于单一职能（如 PM 只写文档、UI 只画页面、UX 只做交互设计），且团队缺少专职 UI 设计师，PM 经常被迫兼任交互与样式设计，既额外占用产出时间又难以保证设计合理性；要做 PM + UI + UX 全栈交付，必须多人协作。Agent 工作流下，**1 人即可同时承担 PM、UI、UX 三类产出**，跨职能沟通次数由 N → 0。
...
| 工具/能力                                                  | 用途                                                        | 选择原因                                                 |
| ------------------------------------------------------ | --------------------------------------------------------- | ---------------------------------------------------- |
| **WorkBuddy 视觉化原型 Skill**（`prototype-design`）          | 根据参考截图输出高保真 HTML 原型，自动沿用项目既有的主色、顶栏结构与表格/弹窗/抽屉等组件风格，可适配任意页面。      | 已内置 58 种设计风格与 Figma 风格默认组件库，可直接复用现有页面 CSS 类名，避免重造组件。 |
...
# 任务
参考我上传的截图，为【页面名】绘制高保真 HTML 原型页面，并同步编写该页面的用户故事（US）与需求说明。

# 约束
1. 保持项目现有风格：沿用既有主色、顶栏结构、左侧导航与组件类名（若项目已有设计约定则自动继承）。
2. 页面需内联 showPage/navUrl 覆盖，保证从其他页面跳转到本页路径正确。
3. 关键模块（列表、弹框、抽屉、批量操作）需挂载 US 说明按钮，点击展开对应用户故事。
4. 每个 US 必须包含完整的「描述、筛选项、字段、数据来源、操作流程、验收标准」六段内容（详见上文术语约定）。
5. 完成后用 agent-browser 验证导航矩阵与关键交互，并更新对应的需求文档截图。
...
| **人力成本**   | 单人只能产出「静态原型」或「纯文本 PRD」二选一；缺少专职 UI 时 PM 需兼任 UI/UX，跨职能需多人协作 | 1 人可同时承担 PM + UI + UX 三类产出，跨职能沟通次数 N → 0 | 产出物覆盖原型 + PRD + 截图 + Git 提交 |
...
| **投放运营 / 业务方** | 需要把复杂业务流程（如多层级管理、批量分配）可视化，便于与研发对齐。    |

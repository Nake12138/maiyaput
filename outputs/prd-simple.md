...
- [ ] 选择「新建」后在弹框内直接展开字段：TikTok账号名称（必填，**最长 30 个字符**）/ 头像（必传，支持预览与清除）/ 国家地区（必选，**下拉枚举值由接口动态下发**）
- [ ] TikTok账号名称输入框限制 `maxlength=30`，超长提交提示「TikTok账号名称长度不能超过30个字符」
- [ ] 国家/地区下拉框选项不硬编码，由「获取地区枚举」接口返回填充（原型以模拟接口实现）
...
- **校验**：新建账号名称非空且 ≤30 字符、头像必传、国家/地区必选（枚举值来自接口），前端 + 后端双重校验；批量分配是否已分配由后端判断，避免前端状态不一致。

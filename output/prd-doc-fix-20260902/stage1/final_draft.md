...
本文档定义商务中心页面的功能需求、交互逻辑、数据筛选、表格字段与接口依赖，作为原型评审、研发排期与测试验收的依据。文档覆盖 7 个核心用户故事、12 个功能点，并明确每个表格的字段说明、数据来源与数据流转路径。
...
查询商务中心列表
  ...
  │      │      └─→ 行内「删除」→ 二次确认「是否取消授权用户该TikTok？」→ 调用撤销接口取消授权
...
2. 可按商务中心名称筛选，点击「查询」后执行列表查询，点击「重置」清空条件并恢复全量列表。
...
|-----------|---------|--------------|--------|------|
| F-001 | 商务中心列表查询 | 商务中心主页面 | P0 | 按名称筛选 + 查询/重置，点击查询后执行 |
| F-002 | 商务中心主表展示 | 商务中心主页面 | P0 | 名称/用户数量/TikTok账号/操作四列 |
| F-003 | 打开用户列表抽屉 | 用户列表抽屉 | P0 | 点击用户数量触发 |
| F-004 | 用户列表搜索 | 用户列表抽屉 | P1 | 按用户名查询/重置，点击查询后执行 |
...
| F-008 | TikTok 账号列表搜索 | TikTok 账号列表抽屉 | P1 | 按账号名称查询/重置，点击查询后执行 |
...
| F-012 | 取消用户 TikTok 账号授权 | 用户已分配 TikTok 账号抽屉 | P0 | 操作列「删除」，二次确认后调用撤销接口 |
...
2. 支持按商务中心名称模糊查询与重置；点击「查询」后才执行列表查询，输入过程中不触发查询。
...
- 点击主表「用户数量」→ 携带 BC ID 打开用户抽屉 → 调用「获取商务中心成员列表」→ 渲染成员列表；输入关键字后点击「查询」执行列表查询，勾选状态独立于可见列表保留。

**验收标准**

1. 点击「用户数量」打开右侧窄抽屉（约 720px），标题：用户列表 - {BC名称}（{用户数}）。
2. 支持按用户名搜索（包含匹配、不区分大小写），点击「查询」后执行查询、点击「重置」恢复全量列表，无结果显示「暂无数据」空态。
...
2. 弹框列出该 BC 全部 TikTok 账号（头像 + 名称 + 语言），支持按名称搜索（点击「查询」后执行）与表头全选。
...
- 语言：下拉选择（占位「请选择语言」），枚举与短剧管理页面的语言枚举一致。
- 备注：单行文本输入框（占位「请输入备注」）。
...
| 字段 | 展示形态 | 说明 |
|------|---------|------|
| TikTok账号名称 | 头像 + 名称 | 组合展示账号信息 |
| 语言 | 文本 | 账号语言 |
| 备注 | 文本 | 账号备注，可为空 |
| 更新人 | 文本 | 最近操作人 |
| 更新时间 | 文本 | 格式 yyyy-MM-dd HH:mm |
| 操作 | 按钮（删除） | 仅「用户已分配 TikTok 账号抽屉」提供：点击后二次确认「是否取消授权用户该TikTok？」，用户确认后调用「撤销用户对资产的权限」接口取消授权；取消则不执行 |

BC 维度展示全部 TikTok 账号资产，无操作列；从用户列表进入（用户已分配 TikTok 账号抽屉）仅展示该用户已分配的账号，操作列提供「删除」，筛选项同样保持一致。

**数据来源（调用接口）**

- 抽屉内账号列表 ← 接口「获取资产列表」([get-assets 官方文档](https://business-api.tiktok.com/portal/docs/get-assets/v1.3))，入参为当前 BC ID。
- 取消授权 ← 接口「撤销用户对资产的权限」([unassign-an-asset 官方文档](https://business-api.tiktok.com/portal/docs/unassign-an-asset/v1.3))。

**数据流转**

- 点击主表「TikTok 账号」或用户列表行内数量 → 携带 BC ID 打开宽抽屉 → 调用「获取资产列表」→ 渲染账号明细；从用户列表进入时叠加在用户抽屉之上，关闭后回到用户列表。
- 用户已分配场景：点击行内「删除」→ 弹出二次确认「是否取消授权用户该TikTok？」→ 用户确认后调用「撤销用户对资产的权限」→ 成功后从列表移除该账号并同步该用户的账号数量；用户取消则不调用接口。

**验收标准**

1. 点击主表或用户列表的「TikTok 账号」数量，打开右侧宽抽屉，标题：TikTok 账号列表 - {BC名称}（{数量}）。
2. 表格字段：TikTok账号名称（头像 + 名称）/ 语言 / 备注 / 更新人 / 更新时间 / 操作；BC 维度无操作列，用户已分配场景操作列为「删除」。
3. 支持按账号名称、语言、备注组合查询（点击「查询」后执行，「重置」恢复全量），无结果显示空态。
4. 从用户列表进入时叠加在用户抽屉之上，关闭后回到用户列表。
5. 用户已分配场景点击「删除」弹出二次确认「是否取消授权用户该TikTok？」；用户确认后调用「撤销用户对资产的权限」接口取消授权，成功后移除该账号并刷新列表，用户取消则不执行。

**界面截图**

![US-005 用户已分配 TikTok 账号列表（含删除操作）](C:/Users/M/WorkBuddy/2026-08-31-18-10-03/maiyaput/assets/prd/us-004-user-tt.png)

![US-005 取消授权二次确认](C:/Users/M/WorkBuddy/2026-08-31-18-10-03/maiyaput/assets/prd/us-004-user-tt-confirm.png)
...
|----------|--------------|---------|---------|
| US-001 | 商务中心名称查询 | 单行文本输入框 + 按钮 | 占位提示「请输入商务中心名称」，点击「查询」后执行查询，「重置」恢复全量 |
| US-002 | 用户名搜索 | 单行文本输入框 + 按钮 | 点击「查询」后执行查询（包含匹配、不区分大小写），「重置」恢复全量 |
| US-003 | 账号搜索 | 单行文本输入框 + 按钮 + 复选框 | 点击「查询」后执行查询；列表首列复选框，表头全选 |
| US-004 | 账号搜索 | 单行文本输入框 + 按钮 + 复选框 | 点击「查询」后执行查询；列表首列固定复选框，表头全选 |
| US-005 | 账号搜索（与资产管理-TikTok账号页面一致） | 单行文本输入框 ×2 + 下拉选择 + 按钮 | 名称/语言/备注三条件，点击「查询」后执行，「重置」恢复全量 |
...
| 单用户分配弹框 | 选择列 / TikTok 账号 / 语言 | 已分配账号禁用并标注；TikTok 账号为头像 + 名称 |
| 批量分配弹框 | 选择列 / TikTok 账号 / 语言 | 复选框固定显示；不展示已分配状态 |
| TikTok 账号列表抽屉 | TikTok账号名称 / 语言 / 备注 / 更新人 / 更新时间 / 操作（用户已分配场景） | BC 维度无操作列；从用户列表进入时仅展示该用户已分配账号，操作列提供「删除」（二次确认后取消授权） |
...
### 9.4 取消用户 TikTok 账号授权（删除）

用户已分配 TikTok 账号抽屉中，点击行内「删除」→ 前端弹出二次确认「是否取消授权用户该TikTok？」→ 用户点击「确定」后调用「撤销用户对资产的权限」(unassign-an-asset)，入参为 BC ID / 用户 / 账号 → 接口成功后从该用户已分配列表移除账号、同步刷新用户列表中的账号数量；用户点击「取消」则不发起调用。
...
- 商务中心成员仅能查看/操作其有权限的商务中心及资产；资产分配、取消授权、账号添加等写操作需校验操作者权限。
...
- 取消授权（删除）：前端二次确认后才调用接口；接口失败时列表不回滚、需给出错误提示。
...
- 取消授权确认弹框 z-index 高于用户已分配 TikTok 账号抽屉，确认/取消后回到抽屉原列表。

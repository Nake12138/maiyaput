...
> **文档版本**：v2.0（对齐左侧商务中心单选 + 右侧 BC用户 / TikTok账号 双页签的最新交互）
> **确认状态**：9 个用户故事（US-001 ~ US-009）待确认
...
4. 在 BC用户列表中可进行单个分配（行内「分配 TikTok 账号」）或批量分配（勾选多个 BC用户后点「分配TikTok」）。
5. 点击 BC用户行内「TikTok 账号」数量，打开该 BC用户已分配账号抽屉，可筛选、单条删除或批量取消授权。
6. 切换到「TikTok账号」页签可查看该 BC 下全部 TikTok 账号资产，并从这里发起「添加 TikTok 账号」。
7. 左侧切换 BC 后，右侧两个页签同时按新 BC 刷新。
...
![US-001 商务中心列表展示与单选](images/us-001-main.png)
...
![US-002 BC用户列表页签与搜索](images/us-002-user-tab.png)
...
- [ ] 弹框列出该 BC 全部 TikTok 账号（头像 + 名称 + 语言），支持按名称搜索与表头全选
...
![US-003 单个 BC用户分配 TikTok 账号](images/us-003-single-assign.png)
...
- [ ] 抽屉仅列出该 BC用户已被分配的 TikTok 账号（头像 + 名称），不展示 BC 下未分配给该 BC用户的账号
...
![US-004 BC用户已分配 TikTok 账号抽屉](images/us-004-user-tt.png)
...
![US-005 批量分配 TikTok 账号](images/us-004-batch-assign.png)
...
![US-006 TikTok 账号列表页签与搜索](images/us-006-tt-tab.png)
...
![US-007 添加 TikTok 账号（新建方式）](images/us-006-add-new.png)
...
- 二维码来自接口「获取 TikTok 账号授权链接」（[obtain-a-tiktok-account-ad-delivery-authorization-url 官方文档](https://business-api.tiktok.com/portal/docs/obtain-a-tiktok-account-ad-delivery-authorization-url/v1.3)）返回的授权链接渲染生成。
...
![US-008 添加 TikTok 账号（关联方式）](images/us-007-add-link.png)
...
![US-009 BC用户已分配 TikTok 账号批量删除](images/us-009-batch-remove.png)
...
| US-007 | 在商务中心创建 TikTok 账号 | <https://business-api.tiktok.com/portal/docs/create-an-organization-account-in-a-business-center/v1.3> |
